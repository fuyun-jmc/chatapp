...
(function () {
  'use strict';
  console.log('[chatapp] app.js build v280 loaded');
  ...
  function onFrAddSearch(forceSubmit) {
    ...
    if (PHONE_RE.test(kw)) {
      ...
      var qb = sb.from('profiles').select('id,phone,nickname,avatar_path,hide_phone').eq('phone', kw);
      if (!state.isDev) qb = qb.eq('hide_phone', false);
      qb.maybeSingle()
        .then(function (r) {
